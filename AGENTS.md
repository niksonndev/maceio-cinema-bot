...
**Maceió Cine Bot** is a Telegram bot that queries the Ingresso.com public API for real-time cinema schedules in Maceió, Brazil. It is a Node.js application (ES Modules, `"type": "module"`).
...
| `npm run bot:listen` | `node src/bot.js` | Local Telegram bot (polling) + Express health check. **Requires** `TELEGRAM_BOT_TOKEN`. |
| `npm test` | Docker Compose | Vitest command suites (Node 22 + LocalStack S3). **Requires Docker**. |
| `npm run sam:build` | `sam build` | Build the SAM application. |
...
| Lambda | `src/lambda.handler` / `src/lambda.fetchHandler` | Production webhook + daily cache warm. |
...
- `sam validate` / `sam build` — SAM template validation and package build
...
- Express health check runs on `PORT` (default `10000`) and is part of `bot.js` (local/dev only).
